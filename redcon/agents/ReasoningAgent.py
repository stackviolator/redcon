...
class ReasoningAgent(IAgent):
    ...
    def run_nmap_scan(self, arguments: dict) -> str:
        ...
        NOTE: subprocess.run() can't have commands chained with operators (`;`, `&&`, `|`) unless shell=True is passed. `nmap` is always appended the subprocess ensuring that the command being ran is `nmap`. However, this could stil be abused.. 

        arguments: dict that maps str -> args
        """
        args = arguments['args']
        cmd = f"nmap {args}".split()
    ...
    def handle_tool_calls(self, tool_calls):
        for call in tool_calls:
            ...
            self.update_short_term_memory(f"Tool call: {call.function.name} {call.function.arguments}\nOutput: {output}")
